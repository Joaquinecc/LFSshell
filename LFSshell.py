...
def shell_demon(state,service):
    ...
    if state == 'up':
        try:
            #p=Popen(["systemctl","start",service], stdin=PIPE, stdout=PIPE, stderr=PIPE)
            #Corre el servicio
            os.system("/etc/init.d/{0} start".format(service))
        ...
        try:
            ...
            os.system("/etc/init.d/{0} stop".format(service))
            write_commands_log("demon"+state+" "+service)            
        except OSError as error:
            print(error) 
            write_error_log(error)
